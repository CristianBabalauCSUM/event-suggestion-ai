import { EventData } from "../definitions";

const getEventScore = (event: EventData, surveyAnswers: { question1: string; question2: string; question3: string }) => {
    let score = 0;
  
    // Compare 'social' answer with event social level
    const socialMap: { [key: string]: number } = { '😶': 1, '😌': 2, '😁': 3 };
    const surveySocial = socialMap[surveyAnswers.question2];
    if (event.social === surveySocial) score += 10; // Higher weight if the social levels match
  
    // Compare 'sentiment' answer with event sentiment
    const sentimentMap: { [key: string]: number } = { '😶': 1, '😌': 2, '😁': 3 };
    const surveySentiment = sentimentMap[surveyAnswers.question1];
    if (event.sentiment === surveySentiment) score += 10;
  
    // Check if the event type matches the user's preferred activity
    const surveyType = surveyAnswers.question3.split(' ')[1]; // Extract 'Sport', 'Meditation', etc.
    if (event.type.includes(surveyType)) score += 15; // Higher weight for activity type match
  
    return score;
  };
  

  const sortEventsBySurvey = (events: EventData[], surveyAnswers: any) => {
    return events
      .map((event) => ({
        ...event,
        score: getEventScore(event, surveyAnswers),
      }))
      .sort((a, b) => b.score - a.score); // Sort by score, highest first
  };